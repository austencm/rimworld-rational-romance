...
namespace RationalRomance {
	...
	public class JobDriver_DoLovinCasual : JobDriver {
		...
		private Pawn Partner {
			get { 
				...
				Pawn p = (Pawn)((Thing)base.job.GetTarget(this.PartnerInd)); 
			...
			if (this.IsTargetPawnFreeForHookup()) {
				...
				if (this.wasSuccessfulPass) {
					yield return new Toil {
						...
						initAction = delegate {
							if (this.wasSuccessfulPass) {
								Job job1 = new Job(RRRJobDefOf.DoLovinCasual, this.TargetPawn, this.TargetBed, this.TargetBed.GetSleepingSlotPos(0));
								this.actor.jobs.jobQueue.EnqueueFirst(job1);
	...
	public class JoyGiver_CasualHookup : JoyGiver {
		public static float percentRate = 0; //RationalRomance.Settings.hookupRate / 2;
	...
	public static class SexualityUtilities {
		public static Pawn FindAttractivePawn(Pawn p1) {
			...
			if (p1.story.traits.HasTrait(RRRTraitDefOf.Asexual)) {
				...
				enumerable = enumerable.Except(from p in enumerable where(p.story.traits.HasTrait(RRRTraitDefOf.Asexual) || !p.RaceProps.Humanlike || (p.story.traits.HasTrait(TraitDefOf.Gay) && p.gender != p1.gender) || (p.story.traits.HasTrait(RRRTraitDefOf.Straight) && p.gender == p1.gender)) && (double)Rand.Value < 0.8 select p);
